...
# Reusing existing VPC vpc-023dc15715418b42e; subnets auto-discovered.
data "aws_subnets" "cluster" {
  filter {
    name   = "vpc-id"
    values = ["vpc-023dc15715418b42e"]
  }
}
...
module "eks" {
  ...
  eks_managed_node_groups = {
    developments-workers = {
      ...
      desired_size   = 2
      disk_size      = 100
      labels = { role = "developments-workers", env = "production" }
    }
  }
